    ...
    : QOpenGLWidget(theParent), myIsCoreProfile(true)
{
    ...
    this->myFilePaths = std::make_shared<std::vector<std::string>>();

    // create viewer
    this->myViewer = new V3d_Viewer(aDriver);
    this->myViewer->SetDefaultBackgroundColor(Quantity_NOC_BLACK);
    this->myViewer->SetDefaultLights();
    this->myViewer->SetLightOn();
    //this->myViewer->ActivateGrid(Aspect_GT_Rectangular, Aspect_GDM_Lines);

    // create AIS context
    this->myContext = new AIS_InteractiveContext(this->myViewer);

    this->myViewCube = new AIS_ViewCube();
    this->myViewCube->SetViewAnimation(this->myViewAnimation);
    this->myViewCube->SetFixedAnimationLoop(false);
    this->myViewCube->SetAutoStartAnimation(true);
    this->myViewCube->TransformPersistence()->SetOffset2d(Graphic3d_Vec2i(100, 150));

    // note - window will be created later within initializeGL() callback!
    this->myView = this->myViewer->CreateView();
    this->myView->SetImmediateUpdate(false);
#if 0
#ifndef __APPLE__
    this->myView->ChangeRenderingParams().NbMsaaSamples = 4; // warning - affects performance
#endif
    this->myView->ChangeRenderingParams().ToShowStats = true;
    this->myView->ChangeRenderingParams().CollectedStats = (Graphic3d_RenderingParams::PerfCounters)
    ...
    if (this->myIsCoreProfile)
    {
        aGlFormat.setVersion(4, 5);
    }
    aGlFormat.setProfile(this->myIsCoreProfile ? QSurfaceFormat::CoreProfile : QSurfaceFormat::CompatibilityProfile);
...
OcctQtViewer::~OcctQtViewer()
{
    ...
    Handle(Aspect_DisplayConnection) aDisp = this->myViewer->Driver()->GetDisplayConnection();

    // release OCCT viewer
    this->myContext->RemoveAll(false);
    this->myContext.Nullify();
    this->myView->Remove();
    this->myView.Nullify();
    this->myViewer.Nullify();
...
void OcctQtViewer::dumpGlInfo(bool theIsBasic, bool theToPrint)
{
    ...
    this->myView->DiagnosticInformation(aGlCapsDict, theIsBasic ? Graphic3d_DiagnosticInfo_Basic : Graphic3d_DiagnosticInfo_Complete);
    ...
    this->myGlInfo = QString::fromUtf8(anInfo.ToCString());
}
...
void OcctQtViewer::initializeGL()
{
    ...
    if (!aGlCtx->Init(this->myIsCoreProfile))
    {
    ...
    Handle(Aspect_NeutralWindow) aWindow = Handle(Aspect_NeutralWindow)::DownCast(this->myView->Window());
    if (!aWindow.IsNull())
    {
        aWindow->SetSize(aViewSize.x(), aViewSize.y());
        this->myView->SetWindow(aWindow, aGlCtx->RenderingContext());
        this->dumpGlInfo(true, true);
    }
    else
    {
        ...
        this->myContext->Display(this->myViewCube, 0, 0, false);
    }
}
...
void OcctQtViewer::paintGL()
{
    ...
    //Handle(OpenGl_GraphicDriver) aDriver = Handle(OpenGl_GraphicDriver)::DownCast (this->myContext->CurrentViewer()->Driver());
    //Handle(OpenGl_Context) aGlCtx = aDriver->GetSharedContext();
    Handle(OpenGl_Context) aGlCtx = OcctGlTools::GetGlContext(this->myView);
    ...
    if (aViewSizeNew != aViewSizeOld)
    {
        ...
        this->dumpGlInfo(true, false);

#if (OCC_VERSION_MAJOR >= 7 && OCC_VERSION_MINOR >= 8 && OCC_VERSION_MAINTENANCE >= 0)
        for (const Handle(V3d_View)& aSubviewIter : this->myView->Subviews())
        {
    ...
    Handle(V3d_View) aView = !this->myFocusView.IsNull() ? this->myFocusView : this->myView;
    aView->InvalidateImmediate();
    FlushViewEvents(this->myContext, aView, true);
}
...
{
    this->myBtnOpen = nullptr;
    this->myListView = nullptr;
    this->myTreeView = nullptr;
    this->mySelectedFiles.clear();

    this->setDirectory(QDir("D:\\Data\\Z_Feature_Recognition"));
    ...
    for (int i = 0; i < btns.size(); ++i)
    {
        ...
        {
            this->myBtnOpen = btns[i];
            break;
        }
    }

    if (!this->myBtnOpen) return;

    this->myBtnOpen->installEventFilter(this);
    this->myBtnOpen->disconnect(SIGNAL(clicked()));
    connect(this->myBtnOpen, SIGNAL(clicked()), this, SLOT(chooseClicked()));

    this->myListView = findChild<QListView*>("listView");
    if (this->myListView)
        this->myListView->setSelectionMode(QAbstractItemView::ExtendedSelection);
    this->myTreeView = findChild<QTreeView*>();
    if (this->myTreeView)
        this->myTreeView->setSelectionMode(QAbstractItemView::ExtendedSelection);
}
...
{
    QModelIndexList indexList = this->myListView->selectionModel()->selectedIndexes();
    foreach(QModelIndex index, indexList)
    {
        if (index.column() == 0)
        {
            this->mySelectedFiles.append(this->directory().absolutePath() + "/" + index.data().toString());
        }
    }
